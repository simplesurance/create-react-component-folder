...
program
  ...
  .option('--storybookweb', 'Creates a storybook web component')
...
/**
 * Creates files for Broker Customer UI
 * @param {String} componentName - Component name
 * @param {String} componentPath - File system path to component
*/
function createFilesBrokerCustomerUI(componentName, componentPath) {
  return new Promise((resolve) => {
    // File extension
    let name = componentName;
...
/**
 * Create files for storybook web
 */
function createFilesStoryBookWeb(componentName, componentPath) {
  return new Promise((resolve) => {
    // File extension
    let name = componentName;
    const indexFile = `${name}.${webExt}`;
    const storybookFile = `${name}.${storybookExt}`;
    const testFile = `${name}.test.js`;

    // file names to create
    const files = [indexFile, storybookFile, testFile];
    ...
    fs
      ...
      .then(() => {
        ...
        for (let i = 0; i < files.length; i += 1) {
          ...
          if (file === indexFile) {
            // Create Index File
            data = createFunctionalStoryBookComponent(name);
            promises.push(fs.writeFileAsync(filePath, data));
          } else if (file === `${storybookFile}`) {
            // Create Web File
            data = createReactStoryBookComponent(name);
            promises.push(fs.writeFileAsync(filePath, data));
          } else if (file === `${testFile}`) {
            data = createStorybookWebTest(name, program.uppercase);
            promises.push(fs.writeFileAsync(filePath, data));
          }
        }
  ...
  fs
    ...
    .then(() => {
      ...
      for (let i = 0; i < args.length; i += 1) {
        ...
        if (program.storybookweb) {
          promises.push(createFilesStoryBookWeb(name, folderPath + name));
        } else {
          promises.push(createFilesBrokerCustomerUI(name, folderPath + name));
